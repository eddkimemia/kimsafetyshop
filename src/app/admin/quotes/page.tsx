...
export default function AdminQuotesPage() {
  ...
  const { data, loading } = useFetch<{ quotes: Quote[] }>("/api/admin/quotes");
  const [notice] = useState<string | null>(params.get("created") ? `Quotation ${params.get("created")} saved — open it below to download the PDF.` : null);
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
          ...
          >
            {quotations.length === 0 ? (
              ...
              <div className="space-y-2">
                {quotations.map((q) => (
                  <QuoteRow key={q.id} q={q} isRfq={false} />
                ))}
              </div>
            )}
          </AdminCard>
...
function QuoteRow({ q, isRfq }: { q: Quote; isRfq: boolean }) {
  return (
    <Link
      href={`/admin/quotes/${encodeURIComponent(q.id)}`}
      className="flex w-full items-center justify-between gap-3 rounded-2xl border border-line bg-surface px-5 py-3.5 text-left transition-colors hover:border-safety-300 hover:bg-safety-50"
    >
