...
7. [~] on vercel some products and category page are not loading, other are giving erors while trying to load individual product/category, give an error then load later or load when tried on a diffrent tab — root cause: `mergedCatalog()` opened fresh connections to the remote Postgres on every render (product page ×2 + client fetch). Fix: 20s TTL cache of admin rows in `src/lib/catalog.ts` (`adminRows()` + `invalidateCatalogCache()`), shared by `src/lib/admin-products.ts`, invalidated on admin product POST/PATCH/DELETE. Build passes locally — PENDING: deploy to Vercel and verify product/category pages load without the first-load error
